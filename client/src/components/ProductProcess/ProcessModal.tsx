    ...
    <Modal theme={customTheme} dismissible show={show} onClose={onClose}>
      ...
      <Modal.Body>
        <div className="flex w-full justify-between">
          {specificProcess?.processData?.start_date && (
            <p>
              Start Date:{" "}
              <b className="text-green-600">
                {getDate(specificProcess?.processData?.start_date)}
              </b>
            </p>
          )}
          {specificProcess?.processData?.end_date && (
            <p>
              End Date:{" "}
              <b className="text-red-600">
                {getDate(specificProcess?.processData?.end_date)}
              </b>
            </p>
          )}
        </div>
        {specificProcess?.processItems &&
        specificProcess.processItems.length > 0 ? (
          ...
          <p className="text-center p-10 text-sm text-gray-400">
      ...
      <Modal.Footer className="flex justify-between">
        <p>
          Price: <b>{specificProcess?.processData?.price?.toFixed(2) || 0}$</b>
        </p>
