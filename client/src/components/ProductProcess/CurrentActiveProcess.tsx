        ...
        <div className="flex flex-col h-full items-start">
          <div className="divide-y w-full flex flex-col justify-between h-full">
              ...
              <div className="h-2/5 w-full">
                <div className="w-full">
                  ...
                  <p className="text-green-600">
                    Start Date:{" "}
                    {getDate(activeProcess.processData?.start_date as Date)}
                  </p>
                </div>
              </div>
                ...
                {activeProcess.processItems.length > 0 ? (
                  activeProcess.processItems.map((m) => (
                    <div
                      key={m._id}
                      color="success"
                      className="w-fit h-fit text-sm border rounded-lg cursor-pointer m-1.5 p-1.5 hover:!bg-green-100"
                    >
                  ...
                  <div className="flex justify-center h-full w-full items-center">
                    <p className="text-gray-400 text-xs">
                      This Process doesn't have any items.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="h-1/6 w-full flex">
              <CustomButton
                className="h-full flex-grow rounded-none bg-gray-300 rounded-bl-lg hover:!bg-gray-400"
                onClick={() =>
                  navigate(`/edit-process/${activeProcess.processData._id}`)
                }
                v={1}
              >
                <FiEdit style={style} />
              </CustomButton>
              {activeProcess.processData.start_date &&
              activeProcess.processData.end_date ? (
                ""
              ) : (
                <CustomButton
                  className="h-full flex-grow bg-red-600 rounded-none !rounded-br-lg hover:!bg-red-700"
