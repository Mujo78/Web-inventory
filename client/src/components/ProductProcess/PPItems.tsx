...
const PPItems: React.FC<{ id: string }> = ({ id }) => {
  ...
  const addMaterial = (e: React.FormEvent) => {
    ...
    const newItem = {
      ...
      quantity:
        Number(formData.quantity) > formData.material_quantity
          ? formData.material_quantity
  ...
  return (
    <>
      <div className="pb-3 ml-5">
        ...
        <p className="text-xs mt-1 font-Rubik text-gray-500">
          You can only add as many quantity as are in stock. Hover material to
          see quantity of material.{" "}
        </p>
      </div>
      <div className="flex flex-col h-5/6 ml-3 gap-8 mt-3">
        <div className="flex h-3/6">
                ...
                (n) =>
                  n.quantity > 0 && (
                    <Tooltip key={n._id} content={n.quantity}>
                      <Button
                        disabled={materialsToAdd.some(
                          (m) => m.material_id === n._id
                        )}
                        color="gray"
          ...
          <div className="border-l flex items-center justify-center border-gray-300 w-2/6 h-2/4">
            {showForm && (
              <form className=" flex flex-col items-start !h-full justify-center">
                <AiOutlineCloseCircle
                  className="ml-auto text-gray-400 transition-all cursor-pointer hover:scale-125 ease-out"
        ...
        <div className="w-full h-1/6">
          ...
          <div className="flex justify-end my-3">
            <Button
              color="gray"
              onClick={skipIt}
              className="mr-3 hover:!text-green-500 focus:!ring-green-500 focus:!text-green-500"
            >
              Skip
            </Button>
