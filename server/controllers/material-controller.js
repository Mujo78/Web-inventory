...
const addMaterial = asyncHandler(async (req, res) => {
  ...
  const { name, supplier_id, quantity, min_quantity, price, unit_of_measure } =
    req.body;

  const supplierWithMaterial = await Material.findOne({
    name: name,
...
const getMaterials = asyncHandler(async (req, res) => {
  ...
  if (searchQuery) {
    ...
    if (!materials)
      return res.status(400).json("There are no materials with such name!");
  ...
  if (!allMaterials)
    return res
      .status(400)
      .json("There are no materials available at this moment!");
...
const editMaterial = asyncHandler(async (req, res) => {
  ...
  const { name, quantity, min_quantity, price, unit_of_measure, supplier_id } =
    req.body;
  ...
  if (
    ...
    supplier_id !== materialToUpdate.supplier_id
  ) {
    const supplierWithMaterial = await Material.findOne({
      name: name,
      supplier_id: supplier_id,
    ...
    if (supplierWithMaterial) {
      res.status(400);
      throw new Error(
        "That material from this supplier is already in database!"
      );
    }
  }
  ...
  if (min_quantity !== materialToUpdate.min_quantity)
  ...
  if (unit_of_measure !== materialToUpdate.unit_of_measure)
  ...
  if (supplier_id !== materialToUpdate.supplier_id) {
    ...
    const priceWithoutPdv = Math.round(
      ((materialToUpdate.price / (1 + oldSupplier.pdv / 100)) * 100) / 100
    );
  ...
  const newOne = await Material.findByIdAndUpdate(req.params.id, updates, {
    new: true,
  });
